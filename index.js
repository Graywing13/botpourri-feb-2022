...
client.on('message', async msg => {
    ...
    if (
        msg.content.trim().substring(0, prefix.length).toLowerCase() !==
            prefix ||
        msg.author.bot
    ) {
        return;
    ...
    const serverQueue = module.exports.getServerQueue(msg);
    ...
    if (client.commands.has(commandName)) {
        ...
        for (const item of client.commands.keys()) {
            reply += `\`${item}\`, `;
        }
    ...
    if (command.args && args.length < command.args) {
        sendCommandUsageInfo.execute(
            msg,
            command,
            'please provide arguments :pray: '
        );
        return;
    }
    try {
        if (command.requiresSameCall) {
            ...
            if (
                ...
                msg.guild.me.voice.channel === undefined
            ) {
                serverQueue.connection = await msg.member.voice.channel.join();
                msg.channel.send(
                    'Joined voice channel `' +
                        msg.guild.me.voice.channel.name +
                        '`'
                );
                ...
                return msg.reply(
                    'You need to be in the same voice channel as botpourri (`' +
                        msg.guild.me.voice.channel.name +
                        '`) to use this command dechu.'
                );
            }
        }
