...
const removeFlagIfFound =
    require('../general_cmd/tools/flag').removeFlagIfFound;
    ...
    execute: async function (msg, serverQueue, args = []) {
        ...
        if (args.length == 0) {
            if (serverQueue.songs.length == 0) {
                return msg.reply(
                    'Please put a song link to start off the queue!'
                );
            }
            if (
                msg.content.match(new RegExp('^b.s*p(lay)?s*$', 'i')) !=
                undefined
            ) {
                return serverQueue.playing
                    ? pause(msg, serverQueue)
                    : resume(msg, serverQueue);
            }
        } else {
            // #TODO: check if targetSong ends in a recognized format
            //        else look up args[all] on ytdl-core and set targetSong to first found song.
        ...
        var play_next = async function () {
            if (!serverQueue.songs.length) {
                ...
                return msg.channel.send(
                    ':camping: There are no more queued songs.'
                );
            }
            verbose(
                'There are `' + serverQueue.songs.length + '` songs in queue.'
            );

            // gets song information and sends it
            const playURL = sendSongInfo(
                msg,
                serverQueue.songs[0],
                hasNowPlayingFlag
            );
            ...
            //   songLength = duration;
            // });
            // TODO refactor... this is kind of a mess lol
            const samplePoint = decideSamplePoint(hasSamplePointField, msg);
            const secondsIn =
                Math.random() *
                samplePoint *
                (songLength > 20 ? songLength - 20 : songLength);
            console.log('seconds in (play.js): ' + secondsIn);
            ...
            dispatcher.on('start', () => {
                verbose(
                    `:yellow_circle: Starting \`${playURL}\` at ${secondsIn.toFixed(
                        2
                    )}s in.`
                );
...
function parsePlayURL(playURL) {
    ...
    function checkAgainst(query, matches) {
        return matches.some(m => {
            return query === m; // TODO triple eqeqeq this
        });
    }
    ...
    if (checkAgainst(parsedURL.hostname, youtubeURLs)) {
        ...
        checkAgainst(
            parsedURL.pathname.slice(
                parsedURL.pathname.indexOf('.') - parsedURL.pathname.length
            ),
            catboxExtensions
        )
    ) {
        ...
        throw new Error(
            `Invalid website: only youtube songs and catbox song links (extensions: ${catboxExtensions.join(
                ', '
            )}) are supported.`
        );
    }
